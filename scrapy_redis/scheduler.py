...
class Scheduler(object):
    ...
    def from_settings(cls, settings):
        ...
        auth = settings.get('REDIS_AUTH',REDIS_AUTH)
        timeout = settings.get('REDIS_TIMEOUT',REDIS_TIMEOUT)
        charset = settings.get('REDIS_CHARSET',REDIS_CHARSET)
        persist = settings.get('SCHEDULER_PERSIST', SCHEDULER_PERSIST)
        queue_key = settings.get('SCHEDULER_QUEUE_KEY', QUEUE_KEY)
        queue_cls = load_object(settings.get('SCHEDULER_QUEUE_CLASS', QUEUE_CLASS))
        dupefilter_key = settings.get('DUPEFILTER_KEY', DUPEFILTER_KEY)
        server = redis.Redis(host = host,port =  port,db = db,password = auth,socket_timeout = timeout,charset = charset)
